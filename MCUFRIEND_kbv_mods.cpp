...
MCUFRIEND_kbv_mods::MCUFRIEND_kbv_mods(int CS, int RS, int WR, int RD, int _RST):Adafruit_GFX(240, 320)
{
...
void MCUFRIEND_kbv_mods::reset(void)
{
    done_reset = 1;
    setWriteDir();
    CTL_INIT();
...
void MCUFRIEND_kbv_mods::WriteCmdData(uint16_t cmd, uint16_t dat) { writecmddata(cmd, dat); }
...
void MCUFRIEND_kbv_mods::pushCommand(uint16_t cmd, uint8_t * block, int8_t N) { WriteCmdParamN(cmd, N, block); }
...
uint16_t MCUFRIEND_kbv_mods::readReg(uint16_t reg, int8_t index)
{
    uint16_t ret;
    uint8_t lo;
    if (!done_reset)
...
uint32_t MCUFRIEND_kbv_mods::readReg32(uint16_t reg)
{
...
uint32_t MCUFRIEND_kbv_mods::readReg40(uint16_t reg)
{
    uint16_t h = readReg(reg, 0);
...
uint16_t MCUFRIEND_kbv_mods::readID(void)
{
    uint16_t ret, ret2;
    uint8_t msb;
...
int16_t MCUFRIEND_kbv_mods::readGRAM(int16_t x, int16_t y, uint16_t * block, int16_t w, int16_t h)
{
...
void MCUFRIEND_kbv_mods::setRotation(uint8_t r)
{
...
void MCUFRIEND_kbv_mods::drawPixel(int16_t x, int16_t y, uint16_t color)
{
...
void MCUFRIEND_kbv_mods::setAddrWindow(int16_t x, int16_t y, int16_t x1, int16_t y1)
{
#if defined(OFFSET_9327)
	if (_lcd_ID == 0x9327) {
...
void MCUFRIEND_kbv_mods::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    int16_t end;
#if defined(SUPPORT_9488_555)
    if (is555) color = color565_to_555(color);
...
void MCUFRIEND_kbv_mods::pushColors(uint16_t * block, int16_t n, bool first)
{
    pushColors_any(_MW, (uint8_t *)block, n, first, 0);
}
void MCUFRIEND_kbv_mods::pushColors(uint8_t * block, int16_t n, bool first)
{
    pushColors_any(_MW, (uint8_t *)block, n, first, 2);   //regular bigend
}
void MCUFRIEND_kbv_mods::pushColors(const uint8_t * block, int16_t n, bool first, bool bigend)
{
    pushColors_any(_MW, (uint8_t *)block, n, first, bigend ? 3 : 1);
}

void MCUFRIEND_kbv_mods::vertScroll(int16_t top, int16_t scrollines, int16_t offset)
{
#if defined(OFFSET_9327)
	if (_lcd_ID == 0x9327) {
...
void MCUFRIEND_kbv_mods::invertDisplay(bool i)
{
    uint8_t val;
...
void MCUFRIEND_kbv_mods::begin(uint16_t ID)
{
